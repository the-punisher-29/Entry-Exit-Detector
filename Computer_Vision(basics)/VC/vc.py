...
def rescaleFrame(frame,scale=0.50):     # by 50% down scaled
...
detector = htm.hand_detector(detectionCon=0.5)


while True:
    success, img = cap.read()
    rescaled_frame=rescaleFrame(img)
    img = detector.find_hands(rescaled_frame)
    lm_list = detector.find_position_of_hands(rescaled_frame, draw = False)
    ...
    prev_time = pres_time
    cv2.putText(img, f"FPS: {str(int(fps))}", (10, 70), cv2.FONT_HERSHEY_PLAIN, 1, (255, 0 , 0), 2)
